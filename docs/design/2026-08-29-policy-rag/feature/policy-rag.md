...
- 授权后的 live 验证：ordinary 无 staff 访问且 BFF 403；staff/admin 可读 SOP 且 BFF 通过；无效身份 401、三角色同步写入 RPC 均拒绝。真实 Embedding 仅计算 1 个变化 Chunk、复用 1 个原向量；双端取消与无障碍答复、员工豁免审批与严重过敏来源均通过。
- 清理：三批运行共创建/删除 9 个临时账号，最终独立 SQL 查得残留 0；正式政策仍为 7 current 文档/16 Chunk，业务表未被测试写入。
- 结论：Feature04 正式关闭。12 项不同 live 检查分批通过，保留早先 429 和一次顾客证据不足及成功重跑的记录，不宣称一次全绿或所有未来模型答复均正确。报告位于网站仓库 `tests/ai/policy-rag-closeout.md`，原始证据位于 `tests/ai/policy-rag-live-evidence.json`；保留设计文档作为政策确认和验证追溯依据。
